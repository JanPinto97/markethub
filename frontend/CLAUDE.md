...
- PostCardComponent — standalone reusable card: header (avatar/initial with consistent HSL color, author name + @handle linking to /profile/:username, relative time "4h ago", community badge for public_community origin, three-dot menu with Edit/Delete/Report by role — closes on outside click via HostListener), body (text with "See more" at 280 chars, image or native video player with "Video unavailable" fallback), footer (like with optimistic update + `liked` visual state, comments toggle). Inline comments section: loads via `/posts/:id/comments`, shows 5 at a time with "Load more", new comment input with auth gate and optimistic add. Per comment/reply: like button with optimistic update + visual `liked` state, Reply button. Replies indented with `border-left` line, render `@parent_username` accent prefix from backend `replyingTo`. Single active reply box per post (`activeReplyCommentId` signal): textarea with 400-char counter, Cancel and Reply buttons, inline error on failure. Delete (author or platform mod/superadmin) for posts/comments/replies via three-dot menu — confirm dialog, 300ms fade-out, `ToastService` "Comment deleted." Hover: subtle box-shadow on card, menus always visible on mobile. Username color/initial from `/shared/utils/color.utils.ts`.
...
- CommunityService — getMyCommunities(), getTopicsByIds(), pinned topics localStorage helpers, getFeed(mode, page, limit), createPost(text, mediaFile?) via FormData, likePost(id), deletePost(id), getComments(postId), addComment(postId, text), addReply(postId, commentId, text, replyingToUserId?), likeComment(postId, commentId), likeReply(postId, commentId, replyId), deleteComment(postId, commentId), deleteReply(postId, commentId, replyId). Exports PostX, PostAuthor, PostCommunity, PostComment (now includes `liked`), FeedResponse interfaces. Uses ApiService for JSON calls + raw HttpClient for FormData/DELETE.
