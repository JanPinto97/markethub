...
- PostRedditCommentSectionComponent — independent section component (no likes anywhere, flat — replies disabled). Inputs: postId, topicSlug, commentCount; Output: commentCountChange. Loads comments via `GET /topics/:slug/posts/:postId/comments` (page=1, limit=10, sorted createdAt desc). Skeleton loading, "No comments yet" empty, retry on error. New-comment box (auth-gated, sign-in placeholder otherwise) with auto-resize textarea + 400-char counter + optimistic add. Three-dot menu (owner/mod/superadmin) with native confirm + `DELETE` endpoint. Local commentCount drives the section title and is emitted to the parent.
